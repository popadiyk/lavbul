    ...
    <h1 class="page-title">
        ...
        @if (Voyager::can('add_'.$dataType->name))
            <a href="{{ route('voyager.'.$dataType->slug.'.create') }}" class="btn btn-success">
                <i class="voyager-plus"></i> Створити нового постачальника
            </a>
        @endif    </h1>
...
@section('content')
    <div class="container-fluid">
        <div class="alert alert-info">
            <strong>Що це?</strong>
            <p> Тут можна переглянути постачальників нашого магазину. А також змінювати їхні данні, якщо у вас є відповідний рівень доступу.</p>
        </div>
    </div>
                        ...
                        <table id="dataTable" class="table table-hover">
                            ...
                            @foreach($dataTypeContent as $data)
                                <tr>
                                    ...
                                    <td>{{$data->email}}</td>
                                    <td>{{ $data->phone }}</td>
                                    <td>@if ($data->type_id == 1) Реалізація @else Факт @endif</td>
